...
REPO_PATH="C:\Users\princ\Documents\Portfolio"
  ...
  if [ "$MAKE_COMMITS" -eq 1 ]; then
    ...
    for ((i = 1; i <= NUM_COMMITS; i++)); do
      ...
      GIT_AUTHOR_DATE="$CURRENT_DATE 12:00:00" GIT_COMMITTER_DATE="$CURRENT_DATE 12:00:00" \
        git commit --allow-empty --date="$CURRENT_DATE 12:00:00" -m "Commit #$i for $CURRENT_DATE: new change"
    done
  fi

  # Skip to a random number of days (3 to 5)
  SKIP_DAYS=$(shuf -i 0-2 -n 1)
